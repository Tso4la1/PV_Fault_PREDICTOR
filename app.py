...
# Both files must sit in the same folder as this app.py (or be committed to
# the GitHub repo alongside it) so Streamlit Cloud can find them.
# @st.cache_resource makes Streamlit load these files ONCE and keep them in
# memory across reruns, instead of re-reading from disk on every click —
# this is what actually keeps CPU usage low on the free tier.
